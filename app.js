...
document.querySelector(".btn-roll").addEventListener("click", function () {
  if (gamePlaying) {
    //1. Random number
    var dice = Math.floor(Math.random() * 6) + 1; //methode pour un chiffre entre 1 et 6 au hasard et arrondi

    //2. Display result of the roll dice
    var diceDOM = document.querySelector(".dice"); //on attribut cet element sous une variable pour ne pas répéter
    diceDOM.style.display = "block"; //from display none to display block
    diceDOM.src = "img_css/dice-" + dice + ".png"; //on selectionne l'attribut src lié à la class dice et on modifie l'image lié au chiffre hasardeux tiré via la variable dice
    ...
    if (dice === 6 && previousDice === 6) {
      ...
      //add score
      roundScore += dice; //on ajoute le résultat si le score est diff de 1
...
function reset() {
  ...
  document.querySelector(".dice").style.display = "none"; //cache l'image du dice via style display
